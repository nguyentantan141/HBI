...
def cal_time_check(time,num_seq,sew,list_point_check):
    output=[]
    point_time=time + df.loc[num_seq-1,'SAM']
...
def cal_wip(num_seq,time,list_point_check):
    # num_seq=i ~ row=i-1
    if df.loc[num_seq-1,'wating'] > 0 and df.loc[num_seq-2,'WIP'] > 0 and df.loc[num_seq-2,'WIP'] <= df.loc[num_seq-1,'wating']:
        cal_time_check(time,num_seq,df.loc[num_seq-2,'WIP'],list_point_check)
        df.loc[num_seq-1,'wating']-=df.loc[num_seq-2,'WIP']
        df.loc[num_seq-1,'sewing']+=df.loc[num_seq-2,'WIP']
        df.loc[num_seq-2,'WIP']=0
        
    if df.loc[num_seq-1,'wating']> 0 and df.loc[num_seq-2,'WIP']> df.loc[num_seq-1,'wating']:
        cal_time_check(time,num_seq,df.loc[num_seq-1,'wating'],list_point_check)
        df.loc[num_seq-2,'WIP']-=df.loc[num_seq-1,'wating']
        df.loc[num_seq-1,'wating']=0
        df.loc[num_seq-1,'sewing']=df.loc[num_seq-1,'headcount']
